...
func (o *GetBackupByIDURL) Build() (*url.URL, error) {
	...
	var _path = "/backups/{backupId}"

	backupID := swag.FormatInt64(o.BackupID)
	if backupID != "" {
		_path = strings.Replace(_path, "{backupId}", backupID, -1)
	} else {
		return nil, errors.New("backupId is required on GetBackupByIDURL")
	}
...
func (o *GetBackupByIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetBackupByIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetBackupByIDURL")
	}
